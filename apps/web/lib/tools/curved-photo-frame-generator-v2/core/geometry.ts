...
import type { CurvedPhotoFrameV2Inputs, CurvedPhotoFrameV2Result } from '../types';
 ...
 */
export function generateFrontPlate(inputs: CurvedPhotoFrameV2Inputs): string {
  ...
  const slotY = plateBottomY - slotH;
  const t1SlotX = teethStartXForSlots - slotClearance;
  const t2SlotX = (teethStartXForSlots + toothWForSlots + toothGapForSlots) - slotClearance;
  const rt2SlotX = plateW - (t2SlotX + slotW);
  const rt1SlotX = plateW - (t1SlotX + slotW);

  cutPaths.push(pathEl(rectPath(t1SlotX, slotY, slotW, slotH)));
  cutPaths.push(pathEl(rectPath(t2SlotX, slotY, slotW, slotH)));
  cutPaths.push(pathEl(rectPath(rt2SlotX, slotY, slotW, slotH)));
  cutPaths.push(pathEl(rectPath(rt1SlotX, slotY, slotW, slotH)));
  ...
  const rowSpacing = clamp(frameSettings.kerfRowSpacingMm, 1.5, 5); // user-controlled
 ...
 */
export function generateSideSupport(inputs: CurvedPhotoFrameV2Inputs, mirrored: boolean = false): string {
  ...
  const viewingAngle = clamp(Number(frameSettings.viewingAngleDeg), 60, 85);
 ...
 */
export function generateFrameFront(inputs: CurvedPhotoFrameV2Inputs): string {
 ...
 */
export function generateFrameBack(inputs: CurvedPhotoFrameV2Inputs): string {
  return svgWrap(1, 1, '');
}
 ...
 */
export function generateStand(inputs: CurvedPhotoFrameV2Inputs): string {
 ...
 * - Right side support
 */
export function generateCombinedLayout(inputs: CurvedPhotoFrameV2Inputs): string {
...
export function generateCurvedPhotoFrameV2(inputs: CurvedPhotoFrameV2Inputs): CurvedPhotoFrameV2Result {
