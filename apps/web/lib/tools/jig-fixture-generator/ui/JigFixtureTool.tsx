...
function getShapeTemplates(t: (key: string) => string) {
  return [
    { id: 'rect', name: t('jig_fixture.shape.rectangle'), svg: null },
    { id: 'circle', name: t('jig_fixture.shape.circle'), svg: null },
    { id: 'rounded-rect', name: t('jig_fixture.shape.rounded_rectangle'), svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 80 30"><rect x="1" y="1" width="78" height="28" rx="4" ry="4" fill="none" stroke="black" stroke-width="0.2" /></svg>`, w: 80, h: 30 },
    { id: 'hexagon', name: t('jig_fixture.shape.hexagon'), svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 60 52"><path d="M15 1 L45 1 L59 26 L45 51 L15 51 L1 26 Z" fill="none" stroke="black" stroke-width="0.2" /></svg>`, w: 60, h: 52 },
    { id: 'pill', name: t('jig_fixture.shape.pill'), svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 80 25"><rect x="1" y="1" width="78" height="23" rx="11.5" ry="11.5" fill="none" stroke="black" stroke-width="0.2" /></svg>`, w: 80, h: 25 },
    { id: 'ticket', name: t('jig_fixture.shape.ticket'), svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 85 30"><path d="M4 1 H81 Q84 1 84 4 V10 Q80 10 80 15 Q80 20 84 20 V26 Q84 29 81 29 H4 Q1 29 1 26 V20 Q5 20 5 15 Q5 10 1 10 V4 Q1 1 4 1 Z" fill="none" stroke="black" stroke-width="0.2" /></svg>`, w: 85, h: 30 },
    { id: 'shield', name: t('jig_fixture.shape.shield'), svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 60"><path d="M5 2 H45 Q48 2 48 5 V24 C48 42 37 54 25 58 C13 54 2 42 2 24 V5 Q2 2 5 2 Z" fill="none" stroke="black" stroke-width="0.2" /></svg>`, w: 50, h: 60 },
    { id: 'diamond', name: t('jig_fixture.shape.diamond'), svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 55 55"><path d="M27.5 1 L54 27.5 L27.5 54 L1 27.5 Z" fill="none" stroke="black" stroke-width="0.2" /></svg>`, w: 55, h: 55 },
    { id: 'arch-top', name: t('jig_fixture.shape.arch_top'), svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 60 40"><path d="M8 39 H52 V20 C52 10 44 2 30 2 C16 2 8 10 8 20 Z" fill="none" stroke="black" stroke-width="0.2" /></svg>`, w: 60, h: 40 },
    { id: 'oval', name: t('jig_fixture.shape.oval'), svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 80 35"><ellipse cx="40" cy="17.5" rx="38" ry="15.5" fill="none" stroke="black" stroke-width="0.2" /></svg>`, w: 80, h: 35 },
    { id: 'label-angle', name: t('jig_fixture.shape.label_angle'), svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 90 30"><path d="M1 5 Q1 1 5 1 H70 L89 15 L70 29 H5 Q1 29 1 25 Z" fill="none" stroke="black" stroke-width="0.2" /></svg>`, w: 90, h: 30 },
    { id: 'scalloped-circle', name: t('jig_fixture.shape.scalloped_circle'), svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 45 45"><path d="M22.5 2.5 C24.5 2.5 26 4.2 27.9 4.7 C29.8 5.2 31.9 4.5 33.5 5.7 C35.1 6.9 35 9.1 36.2 10.7 C37.4 12.3 39.5 13 40 14.9 C40.5 16.8 38.8 18.3 38.8 20.3 C38.8 22.3 40.5 23.8 40 25.7 C39.5 27.6 37.4 28.3 36.2 29.9 C35 31.5 35.1 33.7 33.5 34.9 C31.9 36.1 29.8 35.4 27.9 35.9 C26 36.4 24.5 38.1 22.5 38.1 C20.5 38.1 19 36.4 17.1 35.9 C15.2 35.4 13.1 36.1 11.5 34.9 C9.9 33.7 10 31.5 8.8 29.9 C7.6 28.3 5.5 27.6 5 25.7 C4.5 23.8 6.2 22.3 6.2 20.3 C6.2 18.3 4.5 16.8 5 14.9 C5.5 13 7.6 12.3 8.8 10.7 C10 9.1 9.9 6.9 11.5 5.7 C13.1 4.5 15.2 5.2 17.1 4.7 C19 4.2 20.5 2.5 22.5 2.5 Z" fill="none" stroke="black" stroke-width="0.2" /></svg>`, w: 45, h: 45 },
  ];
}
...
export function JigFixtureTool({ onResetCallback, onGetExportPayload }: JigFixtureToolProps) {
  ...
  const formatMessage = useCallback((key: string, values: Record<string, number | string>) => {
    let message = t(key);
  ...
  const shapeTemplates = useMemo(() => getShapeTemplates(t), [t]);
  ...
  const { svg, meta, warnings } = useMemo(() => {
    ...
    if (!centerLayout && (gridW > usableW + 0.001 || gridH > usableH + 0.001)) {
      warn.push(formatMessage('jig_fixture.ui.warnings.grid_exceeds_usable', { gridW: gridW.toFixed(1), gridH: gridH.toFixed(1) }));
    }
    if (centerLayout && (gridW > opts.bedWidthMm + 0.001 || gridH > opts.bedHeightMm + 0.001)) {
      warn.push(formatMessage('jig_fixture.ui.warnings.grid_exceeds_bed', { gridW: gridW.toFixed(1), gridH: gridH.toFixed(1) }));
    }
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-4">
            <div className="flex items-center justify-between gap-3">
              <div className="text-sm font-medium text-slate-100">{t('jig_fixture.ui.section.laser_bed')}</div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-3">
              <label className="grid gap-1">
                <div className="text-xs text-slate-300">{t('jig_fixture.ui.bed.width_label')}</div>
                <input
                  type="number"
              ...
              <label className="grid gap-1">
                <div className="text-xs text-slate-300">{t('jig_fixture.ui.bed.height_label')}</div>
                <input
                  type="number"
                  className="w-full rounded-md border border-slate-800 bg-slate-950 px-3 py-2 text-sm text-slate-100"
                  value={bedHeightMm}
            ...
            <div className="mt-3">
              <label className="flex items-center gap-2 text-sm text-slate-200">
                ...
                />
                {t('jig_fixture.ui.bed.show_outline')}
              </label>
            </div>
          </div>

          <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-4">
            <div className="text-sm font-medium text-slate-100">{t('jig_fixture.ui.section.object')}</div>

            <div className="mt-3 grid grid-cols-2 gap-3">
              <label className="grid gap-1">
                <div className="text-xs text-slate-300">{t('jig_fixture.ui.object.width_label')}</div>
                <input
                  type="number"
              ...
              <label className="grid gap-1">
                <div className="text-xs text-slate-300">{t('jig_fixture.ui.object.height_label')}</div>
                <input
                  type="number"
                  className="w-full rounded-md border border-slate-800 bg-slate-700 px-3 py-2 text-sm text-slate-400 cursor-not-allowed"
              ...
              <label className="grid gap-1 col-span-2">
                <div className="text-xs text-slate-300">{t('jig_fixture.ui.object.shape_label')}</div>
                <select
                  className="w-full rounded-md border border-slate-800 bg-slate-950 px-3 py-2 text-sm text-slate-100"
                  value={objectShape === 'custom' ? 'custom' : objectShape}
                  onChange={(e) => {
                    const val = e.target.value;
                    const tpl = shapeTemplates.find((t) => t.id === val);
                    if (tpl && tpl.svg) {
                      // Template with SVG
            ...
            <div className="mt-3 rounded-md border border-slate-700 bg-slate-900/50 p-3">
                  ...
                  onChange={(e) => {
                    ...
                    reader.onload = (ev) => {
                      ...
                      if (extracted) {
                        ...
                        alert(t('jig_fixture.ui.errors.extract_contour_failed'));
                      }
                    };
              ...
              {customShapeName && (
                <div className="mt-2 text-xs text-green-400">
                  {formatMessage('jig_fixture.ui.object.custom_using', { name: customShapeName })}
                </div>
              )}
            </div>
          </div>
          ...
          <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-4">
            <div className="text-sm font-medium text-slate-100">{t('jig_fixture.ui.section.layout')}</div>

            <div className="mt-3 grid grid-cols-2 gap-3">
              <label className="grid gap-1">
                <div className="text-xs text-slate-300">{t('jig_fixture.ui.layout.rows')}</div>
                <input
                  type="number"
                  className="w-full rounded-md border border-slate-800 bg-slate-950 px-3 py-2 text-sm text-slate-100"
                  value={rows}
              ...
              <label className="grid gap-1">
                <div className="text-xs text-slate-300">{t('jig_fixture.ui.layout.columns')}</div>
                <input
                  type="number"
                  className="w-full rounded-md border border-slate-800 bg-slate-950 px-3 py-2 text-sm text-slate-100"
                  value={cols}
              ...
              <label className="grid gap-1">
                <div className="text-xs text-slate-300">{t('jig_fixture.ui.layout.spacing_x')}</div>
                <input
                  type="number"
                  className="w-full rounded-md border border-slate-800 bg-slate-950 px-3 py-2 text-sm text-slate-100"
                  value={spacingXmm}
              ...
              <label className="grid gap-1">
                <div className="text-xs text-slate-300">{t('jig_fixture.ui.layout.spacing_y')}</div>
                <input
                  type="number"
                  className="w-full rounded-md border border-slate-800 bg-slate-950 px-3 py-2 text-sm text-slate-100"
                  value={spacingYmm}
              ...
              <label className="grid gap-1">
                <div className="text-xs text-slate-300">{t('jig_fixture.ui.layout.margin')}</div>
                <input
                  type="number"
                  className="w-full rounded-md border border-slate-800 bg-slate-950 px-3 py-2 text-sm text-slate-100"
                  value={marginMm}
            ...
            <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
              <label className="flex items-center gap-2 text-sm text-slate-200">
                ...
                />
                {t('jig_fixture.ui.layout.center')}
              </label>
              <label className="flex items-center gap-2 text-sm text-slate-200">
                <input
                  type="checkbox"
                ...
                />
                {t('jig_fixture.ui.layout.cut_holes')}
              </label>
              <label className="flex items-center gap-2 text-sm text-slate-200">
                <input
                  type="checkbox"
                ...
                />
                {t('jig_fixture.ui.layout.engrave_outline')}
              </label>
              <label className="flex items-center gap-2 text-sm text-slate-200">
                <input
                  type="checkbox"
                ...
                />
                {t('jig_fixture.ui.layout.number_positions')}
              </label>
            </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              >
                {t('jig_fixture.ui.actions.export_svg')}
              </button>
              <button
                type="button"
                ...
                title={t('jig_fixture.ui.actions.copy_layout_title')}
              >
                {t('jig_fixture.ui.actions.copy_layout')}
              </button>
              <button
                type="button"
                onClick={async () => {
              ...
              >
                {t('jig_fixture.ui.actions.add_to_price_calculator')}
              </button>
            </div>

            <div className="mt-3 text-xs text-slate-400">
              {formatMessage('jig_fixture.ui.grid_summary', {
                gridW: meta.gridWidthMm.toFixed(1),
                gridH: meta.gridHeightMm.toFixed(1),
                count: rows * cols,
              })}
            </div>
          </div>
        ...
        <div className="min-w-0 overflow-hidden sticky top-4 self-start">
          <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-4">
            <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
              <div className="text-sm font-medium text-slate-100">{t('jig_fixture.ui.section.preview')}</div>
