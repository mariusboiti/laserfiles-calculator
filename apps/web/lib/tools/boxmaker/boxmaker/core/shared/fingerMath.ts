...
/**
 * Compute finger joint pattern for a given edge length.
 * Pattern starts AND ends with a tab (avoids floating corners).
 * @param lengthMm - edge length in mm
 * @param desiredFingerWidthMm - target finger width
 * @param invert - if true, start with gap instead of tab
 */
export function computeFingerPattern(
  lengthMm: number,
  desiredFingerWidthMm: number,
  invert: boolean
): FingerPattern {
