  ...
  return (
    <div className="grid grid-cols-[280px_1fr] gap-4 h-full">
      ...
      <div className="flex flex-col gap-2 p-4 bg-slate-950 rounded-lg">
          ...
          {['all', ...panelKeys].map((panel) => (
            <button
              ...
              className={`px-3 py-1 text-xs rounded ${
                activePanel === panel ? 'bg-sky-600 text-white' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'
              }`}
            >
              {panel}
            </button>
        ...
        {validationError && (
          <div className="px-3 py-2 bg-red-950 border border-red-800 rounded text-red-300 text-xs">{validationError}</div>
        )}

        {warnings.length > 0 && (
          <div className="px-3 py-2 bg-amber-950 border border-amber-800 rounded text-amber-200 text-xs">
        ...
        <div className="flex-1 bg-white rounded-lg overflow-auto p-4">
          {svgContent && activePanel === 'all' && <div dangerouslySetInnerHTML={{ __html: svgContent }} className="w-full h-full" />}
          {panelSvgs && activePanel !== 'all' && panelSvgs[activePanel] && (
            <div className="relative w-full h-full">
              <div className="absolute inset-0 [&_svg]:h-full [&_svg]:w-full [&_svg]:block" dangerouslySetInnerHTML={{ __html: panelSvgs[activePanel] }} />
              {(() => {
