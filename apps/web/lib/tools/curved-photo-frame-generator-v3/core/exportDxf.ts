// Minimal DXF export for the Curved Photo Frame Generator V2.
...
function parseSvgPathsByLayer(svgText: string): { layer: string; dList: string[] }[] {
  ...
  try {
    ...
    // V2 uses groups with id="layer-layername"
    const groups = Array.from(doc.querySelectorAll('g[id^="layer-"]'));
    for (const g of groups) {
      const id = g.getAttribute('id') || '';
      const layerName = id.replace('layer-', '').toUpperCase();
      const layer = layerName === 'CUT' ? 'CUT' : layerName === 'SCORE' ? 'SCORE' : 'ENGRAVE';
...
export function exportCurvedPhotoFrameV3Dxf(svgText: string): string {
  const parts: string[] = [];
