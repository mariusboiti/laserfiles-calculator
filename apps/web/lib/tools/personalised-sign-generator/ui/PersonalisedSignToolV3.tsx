  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 p-4 min-h-0">
      ...
      <div className="w-full lg:w-[400px] lg:min-w-[400px] lg:flex-shrink-0 space-y-4 overflow-y-auto max-h-[calc(100vh-120px)]">
        {/* Shape Section */}
        <Section title="Shape" icon={<Circle className="w-4 h-4" />} expanded={expandedSections.shape} onToggle={() => toggleSection('shape')}>
          <div className="space-y-3">
            ...
            {(config.shape === 'rounded-rect' || config.shape === 'rounded-arch' || config.shape === 'plaque') && (
              <NumberInput label="Corner Radius" value={config.cornerRadius} onChange={(v) => updateConfig({ cornerRadius: v })} min={0} max={LIMITS.cornerRadius.max} />
            )}
        ...
        <Section title="Text" icon={<Type className="w-4 h-4" />} expanded={expandedSections.text} onToggle={() => toggleSection('text')}>
          <div className="space-y-4">
            <TextLineControls label="Line 1 (Top)" line="line1" config={config.text.line1} onChange={(u) => updateText('line1', u)} />
            <TextLineControls label="Line 2 (Main)" line="line2" config={config.text.line2} onChange={(u) => updateText('line2', u)} />
            
            {/* Curved Text for Line 2 */}
            <div className="pl-3 border-l-2 border-slate-700 space-y-2">
              <label className="block text-xs text-slate-400">Curved Text</label>
              <div className="flex gap-2">
                {(['straight', 'arcUp', 'arcDown'] as CurvedMode[]).map((mode) => (
                  ...
                  >
                    {mode === 'straight' ? 'Straight' : mode === 'arcUp' ? 'Arc Up' : 'Arc Down'}
                  </button>
                ))}
              </div>
              {config.text.curvedModeLine2 !== 'straight' && (
                <NumberInput label="Curve Intensity" value={config.text.curvedIntensity} onChange={(v) => setConfig((prev) => ({ ...prev, text: { ...prev.text, curvedIntensity: v } }))} min={0} max={100} />
              )}
            </div>

            <TextLineControls label="Line 3 (Bottom)" line="line3" config={config.text.line3} onChange={(u) => updateText('line3', u)} />

            <NumberInput label="Line Height" value={config.text.lineHeight} onChange={(v) => setConfig((prev) => ({ ...prev, text: { ...prev.text, lineHeight: v } }))} min={0.8} max={1.6} step={0.1} />
            <NumberInput label="Padding (mm)" value={config.padding} onChange={(v) => updateConfig({ padding: v })} min={0} max={60} />
          </div>
        </Section>

        {/* Holes Section */}
        <Section title="Mounting Holes" icon={<Circle className="w-4 h-4" />} expanded={expandedSections.holes} onToggle={() => toggleSection('holes')}>
          <div className="space-y-3">
            <div>
            ...
            {config.holes.preset !== 'none' && (
              <>
                <NumberInput label="Diameter (mm)" value={config.holes.diameter} onChange={(v) => updateHoles({ diameter: v })} min={2} max={20} />
                <NumberInput label="Margin (mm)" value={config.holes.margin} onChange={(v) => updateHoles({ margin: v })} min={0} max={60} />
                {config.holes.preset === 'slots' && (
                  <>
                    <NumberInput label="Slot Length (mm)" value={config.holes.slotLength || 15} onChange={(v) => updateHoles({ slotLength: v })} min={5} max={30} />
                    <NumberInput label="Slot Width (mm)" value={config.holes.slotWidth || 4} onChange={(v) => updateHoles({ slotWidth: v })} min={2} max={10} />
                  </>
                )}
              </>
            )}
        ...
        {/* Decoration Section */}
        <Section title="Icons & Decoration" icon={<Palette className="w-4 h-4" />} expanded={expandedSections.decoration} onToggle={() => toggleSection('decoration')}>
          <div className="space-y-3">
            <div>
              ...
              <>
                <div>
                  ...
                  <select value={config.icon.placement} onChange={(e) => updateIcon({ placement: e.target.value as IconPlacement })} className="w-full bg-slate-800 border border-slate-700 rounded px-2 py-1.5 text-sm">
                    <option value="left-of-line2">Left of Line 2</option>
                    <option value="right-of-line2">Right of Line 2</option>
                    <option value="above-line2">Above Line 2</option>
                    <option value="between-lines">Between Lines</option>
                  </select>
                </div>
                <NumberInput label="Size (mm)" value={config.icon.size} onChange={(v) => updateIcon({ size: v })} min={5} max={100} />
              </>
            )}
            ...
            <div className="border-t border-slate-700 pt-3 mt-3">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs text-slate-300">Enable Monogram</span>
              </div>
              {config.monogram.enabled && (
                <div className="space-y-2 pl-4">
                  ...
                  />
                  <NumberInput label="Size (mm)" value={config.monogram.size} onChange={(v) => updateMonogram({ size: v })} min={10} max={150} />
                  <div>
        ...
        <Section title="Output Settings" icon={<Settings2 className="w-4 h-4" />} expanded={expandedSections.output} onToggle={() => toggleSection('output')}>
          <div className="space-y-3">
            ...
            <NumberInput label="Cut Stroke Width" value={config.output.cutStrokeWidth} onChange={(v) => updateOutput({ cutStrokeWidth: v })} min={0.01} max={0.5} step={0.01} />
            <NumberInput label="Engrave Stroke Width" value={config.output.engraveStrokeWidth} onChange={(v) => updateOutput({ engraveStrokeWidth: v })} min={0.01} max={0.5} step={0.01} />
          </div>
        </Section>

        {/* Sheet Section */}
        <Section title="Sheet Layout" icon={<LayoutGrid className="w-4 h-4" />} expanded={expandedSections.sheet} onToggle={() => toggleSection('sheet')}>
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <input type="checkbox" checked={config.sheet.enabled} onChange={(e) => updateSheet({ enabled: e.target.checked })} className="rounded" />
              <span className="text-xs text-slate-300">Enable Sheet Layout</span>
            </div>
            {config.sheet.enabled && (
              <>
                <div>
                  <label className="block text-xs text-slate-400 mb-1">Sheet Preset</label>
                  <select
                    onChange={(e) => {
                ...
                <div className="grid grid-cols-2 gap-2">
                  <NumberInput label="Width (mm)" value={config.sheet.width} onChange={(v) => updateSheet({ width: v })} min={100} max={3000} />
                  <NumberInput label="Height (mm)" value={config.sheet.height} onChange={(v) => updateSheet({ height: v })} min={100} max={2000} />
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <NumberInput label="Margin" value={config.sheet.margin} onChange={(v) => updateSheet({ margin: v })} min={0} max={50} />
                  <NumberInput label="Spacing" value={config.sheet.spacing} onChange={(v) => updateSheet({ spacing: v })} min={0} max={50} />
                </div>
                <div className="flex items-center gap-2">
                  <input type="checkbox" checked={config.sheet.autoFill} onChange={(e) => updateSheet({ autoFill: e.target.checked })} className="rounded" />
                  <span className="text-xs text-slate-300">Auto-fill sheet</span>
                </div>
              </>
            )}
          </div>
        ...
        {/* AI Section */}
        <Section title="AI Sign Generator" icon={<Wand2 className="w-4 h-4" />} expanded={expandedSections.ai} onToggle={() => toggleSection('ai')}>
          <div className="space-y-3">
            <textarea
              value={aiPrompt}
              onChange={(e) => setAiPrompt(e.target.value)}
              placeholder="Describe your sign... e.g. 'Family name sign for the Smiths, established 2020, with a heart icon'"
            ...
            >
              ...
              {aiLoading ? 'Generating...' : 'Generate Sign'}
            </button>
          </div>
        </Section>
      </div>
...
function TextLineControls({ label, line, config, onChange }: { label: string; line: string; config: SignConfigV3['text']['line1']; onChange: (u: Partial<SignConfigV3['text']['line1']>) => void }) {
  return (
    <div className="space-y-2">
      ...
      <div>
        <label className="block text-[10px] text-slate-400 mb-1">Horizontal Align</label>
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <label className="text-[10px] text-slate-400">Offset X</label>
      ...
      {/* Vertical Alignment */}
      <div>
        <label className="block text-[10px] text-slate-400 mb-1">Vertical Align</label>
        <div className="flex gap-1">
          <button
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <label className="text-[10px] text-slate-400">Offset Y</label>
