import { computeHmacSha256Hex, computeWpSsoSignatureHex, secureCompareHex } from './wp-hmac';

describe('wp-hmac', () => {
  it('computes deterministic hmac sha256 hex', () => {
  ...
  it('secureCompareHex is constant-time style and checks equality', () => {
    const a = 'a'.repeat(64);
    const b = 'a'.repeat(64);
    const c = 'b'.repeat(64);
  ...
  it('computes wp sso signature payload as wpUserId|email|iat', () => {
    ...
    const expected = computeHmacSha256Hex('123|user@example.com|1730000000', 'sso-secret');
